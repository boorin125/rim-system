...
export class TemplateService {
  ...
  async generateTemplate(): Promise<Buffer> {
    ...
    ['A1', 'B1', 'C1'].forEach(cellAddress => {
      const cell = dataSheet.getCell(cellAddress);
      cell.font = { bold: true, size: 11, color: { argb: 'FFFF0000' } };
    });
    
    // ✅ Fixed: Use getCell() instead of column.getCell()
    // Add data validation for dropdowns
    // Store Type dropdown (Column D = storeType)
    for (let i = 2; i <= 1001; i++) {
      const storeTypeCell = dataSheet.getCell(`D${i}`);
    ...
    // Store Status dropdown (Column AV = 48th column = storeStatus)
    for (let i = 2; i <= 1001; i++) {
      const statusCell = dataSheet.getCell(`AV${i}`);
    ...
    const fieldGuide = [
      ...
      { field: 'district', required: 'No', format: 'Text', example: 'Pathumwan', description: 'District/Sub-district' },
      { field: 'province', required: 'No', format: 'Text', example: 'Bangkok', description: 'Province name' },
