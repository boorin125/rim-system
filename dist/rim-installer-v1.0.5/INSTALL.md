...
Installer จะถามข้อมูลเพียง 2 อย่าง:
...
2. ระบบจะพาไปหน้า **Setup** อัตโนมัติ (ครั้งแรกที่ยังไม่มี Admin)
3. **กรอกข้อมูล Super Admin** (ชื่อ, Email, Password) แล้วกด **สร้างบัญชีและเริ่มใช้งาน**
4. **Login** ด้วยบัญชีที่เพิ่งสร้าง
5. **Activate License**: ไปที่ **Settings → License** → ใส่ License Key → กด **Activate**
6. **ตั้งค่าองค์กร**: **Settings → ทั่วไป** → ใส่ชื่อองค์กร, โลโก้
7. **Restore Backup** (ถ้ามีข้อมูลจาก server เดิม): **Settings → Backup → Restore**
...
```
**หมายเหตุ**: Installer v1.0.5 แก้ปัญหานี้แล้ว — จะเกิดก็ต่อเมื่อใช้ installer เวอร์ชั่นเก่า
...
VAPID=$(docker run --rm --entrypoint node rubjobb/rim-backend:1.0.5 \
