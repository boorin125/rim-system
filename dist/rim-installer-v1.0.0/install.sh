...
# ── Check Docker ──────────────────────────────
echo -e "${YELLOW}[1/6] ตรวจสอบ Docker...${NC}"
...
# ── Generate .env ─────────────────────────────
echo -e "${YELLOW}[3/6] สร้างไฟล์ตั้งค่า...${NC}"
...
# Generate VAPID keys via web-push inside backend container (after docker compose up)
...
# ── Pull & Start ──────────────────────────────
echo -e "${YELLOW}[4/6] ดาวน์โหลด Docker images และเริ่มต้นระบบ...${NC}"
echo ""

$COMPOSE_CMD pull
$COMPOSE_CMD up -d

# ── Generate VAPID keys จาก backend container ──
echo -e "${YELLOW}→ สร้าง VAPID keys...${NC}"
sleep 5
VAPID=$(docker exec rim-backend node -e "const wp=require('web-push');const k=wp.generateVAPIDKeys();console.log(k.publicKey+'|||'+k.privateKey);" 2>/dev/null || echo "")
if [ -n "$VAPID" ]; then
