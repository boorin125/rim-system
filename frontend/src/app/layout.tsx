  ...
  return (
    <html lang="th" suppressHydrationWarning>
      <head>
        ...
        <script dangerouslySetInnerHTML={{ __html: `(function(){try{var t=localStorage.getItem('colorTheme')||'dark';document.documentElement.className=t;var s=localStorage.getItem('themeStyle');if(s){var st=JSON.parse(s);var hex=st.bgEnd.replace('#','');var r=parseInt(hex.slice(0,2),16)/255,g=parseInt(hex.slice(2,4),16)/255,b=parseInt(hex.slice(4,6),16)/255;var mx=Math.max(r,g,b),mn=Math.min(r,g,b),h=0,s2=0,l=(mx+mn)/2;if(mx!==mn){var d=mx-mn;s2=l>0.5?d/(2-mx-mn):d/(mx+mn);if(mx===r)h=((g-b)/d+(g<b?6:0))/6;else if(mx===g)h=((b-r)/d+2)/6;else h=((r-g)/d+4)/6;}var pct=t==='dark'?42:62;var hval=Math.round(h*360),sval=Math.max(Math.round(s2*100),30);document.documentElement.style.setProperty('--theme-highlight','hsl('+hval+','+sval+'%,'+pct+'%)');document.documentElement.style.setProperty('--theme-highlight-hover','hsl('+hval+','+sval+'%,'+Math.min(pct+8,75)+'%)');}}catch(e){document.documentElement.className='dark';}})()` }} />
        {/* PWA */}
