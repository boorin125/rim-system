...
export async function generatePmReportPDF(data: PmReportData): Promise<void> {
  ...
  doc.text(`(${data.technicianName || '______________________'})`, marginL + colW / 2, lineY + 4, { align: 'center' })
  doc.setFont(font, 'normal')
  doc.setFontSize(6)
  ...
  doc.setFontSize(7)
  doc.setTextColor(50, 50, 50)
  doc.text(`(${data.storeSignerName || '______________________'})`, rightX + colW / 2, lineY + 4, { align: 'center' })
  doc.setFont(font, 'normal')
  doc.setFontSize(6)
  doc.setTextColor(120, 120, 120)
